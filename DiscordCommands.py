...
async def GetMTScore(idMT: int, ret_soup = False) :
    ...
    if len(tds) <= 4 : score = 2 # Identifiant non attribué
    elif tds[1].getText().strip() in ("Administrateur", "Administratrice"): score = 1 # Administrateur
    elif len(tds) > 4: score = int(tds[4].getText().strip().replace('\u2009', ''))
...
async def info(ctx, idMT: MTid = None):
    ...
    embed = Embed(title=f"{Infos[0]} - {Infos[1]} :flag_{country}:", url=url, description="Membre n°"+str(idMT)+"Rang : "+Infos[6]+"  Top  "+Infos[8]+(7-len(Infos[6]+Infos[8]))*' ' +" <:gold:836978754454028338> : "+Infos[9]+" <:silver:836978754433319002> : "+Infos[10]+" <:bronze:836978754467135519> : "+Infos[11]+" <:mh:836978314387259442> : "+Infos[12], color=int(soup.find('td').find_all('span')[-1]['style'].split('#')[1].split(';')[0], 16))
